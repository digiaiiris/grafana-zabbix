...
dist-backend-%:
	$(eval filename = iiris_zabbix-plugin_$*_amd64$(extension))
...
dist-arm-linux-arm-v6:
	env CGO_ENABLED=0 GOOS=linux GOARCH=arm GOARM=6 go build -ldflags="-s -w" -o ./dist/iiris_zabbix-plugin_linux_arm ./pkg
dist-arm-linux-arm-v7:
	env CGO_ENABLED=0 GOOS=linux GOARCH=arm GOARM=7 go build -ldflags="-s -w" -o ./dist/iiris_zabbix-plugin_linux_arm ./pkg
dist-arm-%-arm64:
	$(eval filename = iiris_zabbix-plugin_$*_arm64$(extension))
...
zip:
	cp -r dist/ iiris-zabbix-app
	zip -r iiris-zabbix-app.zip iiris-zabbix-app
	rm -rf iiris-zabbix-app
